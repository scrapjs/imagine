'use strict';(function() {
  ...
  function L(a, b, c) {
    for(var d = 0, e = 0, g = "", f = 0, k = -1, l = c.length;f < a.length;) {
      if(0 > k) {
        for(var h = 0;h < l;h++) {
          if(a.substr(f, c[h][0].length) === c[h][0]) {
            k = h;
            break
          }
        }
      }
      0 <= k && a.substr(f, c[k][0].length) === c[k][0] && (c[k][0] !== c[k][1] || c[k][0] === c[k][1] && 0 === d) ? (0 === d && (g += a.slice(e, f), e = f + c[k][0].length), f += c[k][0].length, d++) : 0 <= k && 0 < d && a.substr(f, c[k][1].length) === c[k][1] ? (d--, 0 === d ? (g += c[k][0] + b(a.slice(e, f)) + c[k][1], e = f += c[k][1].length, k = -1) : f += c[k][1].length) : f++
    }
  ...
  function O(a, b) {
    ...
    this.fn = P[this.name];
    this.fn || (console.error("no filter `" + this.name + "` found. `none` is used instead."), this.fn = C)
  }
  function G(a, b) {
  ...
  y.prototype = {dataRegExp:RegExp([B[0], "[ ]*([a-zA-Z_$@][a-zA-Z_$@.0-9]*)[^", B[0], B[1], "]*", B[1]].join(""), "ig"), parseDescriptor:function(a) {
  ...
  var P = {capitalize:H, capfirst:H, "default":function(a, b) {
    return a ? a : b
  }, truncatechars:function(a, b, c) {
    ...
    return a.join(" ")
  }, escape:Q, e:Q, any:r, random:r, add:function(a, b) {
    return a + b
  }, cut:function(a, b) {
    return a.replace(RegExp(b, "g"), "")
  }}, R = {"int":m, number:p, "float":J, bool:K, none:C, numeric:function(a, b, c) {
  ...
  }, escapeSymbols:D, unescapeSymbols:E, parseArguments:A, any:r, "int":m, "float":J, number:p, bool:K, replacements:function(a, b) {
    for(var c in b) {
